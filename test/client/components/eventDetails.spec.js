...
describe('EventDetails Component', () => {
  ...
  it('should display the title of the event', () => {
    const paragraph = enzymeWrapper.find('p');
    expect(paragraph).to.have.length(6);
    expect(paragraph.at(2).text()).to.equal('Title: Pokemongodb party')
  })

  it('should display the description of the event', () => {
    const paragraph = enzymeWrapper.find('p');
    expect(paragraph).to.have.length(6);
    expect(paragraph.at(3).text()).to.equal('Description: Catch pokemon and do some coding');
  })

  it('should display the location of the event', () => {
    const paragraph = enzymeWrapper.find('p');
    expect(paragraph).to.have.length(6);
    expect(paragraph.at(4).text()).to.equal('Location: 701 Brazos St, Austin, TX 78701');
  })

  it('should display the time of the event', () => {
    const paragraph = enzymeWrapper.find('p');
    expect(paragraph).to.have.length(6);
    expect(paragraph.at(5).text()).to.equal('Time: 2016-08-30T08:00:00.000')
  })
