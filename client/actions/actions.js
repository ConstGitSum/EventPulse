...
import { parseDuration } from '../utils/form';
...
export function filterList(eventList, filter, userId, hiddenEvents, location) {
  ...
  if (filter === 'unhidden') {
    filteredList = eventList.filter(e => !hiddenEvents.includes(e.id));
  } else if (filter === 'hidden') {
    filteredList = eventList.filter(e => hiddenEvents.includes(e.id));
  } else if (filter === 'created') {
    filteredList = eventList.filter(e => e.created_by === userId);
  } else if (filter === 'joined') {
    filteredList = eventList.filter(e => e.guests.some(guest =>
...
export function getHiddenEvents(userId) {
  const request = axios.get(`/api/events/hide/${userId}`);
...
export function createEvent(formData, currentUser) {
  ...
  if (formData.is_tomorrow === 'true') {
    if (Number(formData.hour) === 12) {
      date = date + (((Number(formData.hour) + 12) - newDate.getHours()) * 3600000) +
        ((Number(formData.minute) - newDate.getMinutes()) * 60000);
    } else {
      date = date + (((Number(formData.hour) + 24) - newDate.getHours()) * 3600000) +
        ((Number(formData.minute) - newDate.getMinutes()) * 60000);
    }
  }

  if (formData.ampm === 'pm') {
    const newHour = Number(formData.hour) === 12
      ? Number(formData.hour)
      : Number(formData.hour) + 12;
    date = date + ((newHour - newDate.getHours()) * 3600000) +
      ((Number(formData.minute) - newDate.getMinutes()) * 60000);
  } else {
    date = date + ((Number(formData.hour) - newDate.getHours()) * 3600000) +
      ((Number(formData.minute) - newDate.getMinutes()) * 60000);
  }
...
export function updateEvent(updatedEvent, currentUser, eventId) {
  ...
  if (updatedEvent.is_tomorrow === 'true') {
    if (Number(updatedEvent.hour) === 12) {
      date = date + (((Number(updatedEvent.hour) + 12) - newDate.getHours()) * 3600000) +
        ((Number(updatedEvent.minute) - newDate.getMinutes()) * 60000);
    } else {
      date = date + (((Number(updatedEvent.hour) + 24) - newDate.getHours()) * 3600000) +
        ((Number(updatedEvent.minute) - newDate.getMinutes()) * 60000);
    }
  }

  if (updatedEvent.ampm === 'pm') {
    const newHour = Number(updatedEvent.hour) === 12
      ? Number(updatedEvent.hour)
      : Number(updatedEvent.hour) + 12;
    date = date + ((newHour - newDate.getHours()) * 3600000) +
      ((Number(updatedEvent.minute) - newDate.getMinutes()) * 60000);
  } else {
    date = date + ((Number(updatedEvent.hour) - newDate.getHours()) * 3600000) +
      ((Number(updatedEvent.minute) - newDate.getMinutes()) * 60000);
  }
